...
def interpolate_files_linear(uniform_folder_path, nonuniform_folder_path, output_folder_path):
    r2_scores = []
    mean_square_errors = []
    ...
    for file in t1:
        ...
        for i in df.columns:
            conv = df[i].to_numpy()
            interp_column = np.interp(uniform_time, non_time, conv)
...
if __name__ == "__main__":
    interpolate_files_linear("D:\\Interpolationnnn\\uniform_all\\",
                 "D:\\Interpolationnnn\\non_uniform_all\\",
                 "D:\\Interpolationnnn\\interpolated_files_lin\\")
